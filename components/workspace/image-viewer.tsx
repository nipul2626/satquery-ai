...
import type { Citation, Scene } from "@/lib/types"
...
function BoundingOverlay({
  ...
  const boxed = citations.filter((c) => c.bbox || c.overlay)
  ...
  return (
      ...
      >
        {boxed.map((citation, index) => {
          ...
          /*
           * Special geometry:
           * Used for things such as roads, bridges, rivers and
           * other elongated features that should not be represented
           * by a generic rectangle.
           */
          if (geometry) {
              ...
              return (
                  <g key={citation.id}>
                    ...
                    <motion.polyline
                        ...
                        strokeWidth={
                          active
                              ? (geometry.strokeWidth ?? 1.8) * 1.35
                              : geometry.strokeWidth ?? 1.8
                        }
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        vectorEffect="non-scaling-stroke"
                    ...
                    {geometry.points.length > 1 && (
                        <>
                          <circle
                              cx={geometry.points[0].x * 100}
                              cy={geometry.points[0].y * 100}
                              r={active ? 1.2 : 0.8}
                              fill={accent}
                              vectorEffect="non-scaling-stroke"
                          />
                          <circle
                              cx={
                                  geometry.points[geometry.points.length - 1].x * 100
                              }
                              cy={
                                  geometry.points[geometry.points.length - 1].y * 100
                              }
                              r={active ? 1.2 : 0.8}
                              fill={accent}
                              vectorEffect="non-scaling-stroke"
...
function OverlayLabels({
  ...
  const visible = citations.filter((c) => c.bbox || c.overlay)
  ...
  return (
      <div className="pointer-events-none absolute inset-0 z-30">
        {visible.map((citation) => {
          ...
          /*
           * For custom line geometry, place the label near the
           * beginning/upper portion of the feature rather than
           * at the old rectangle's top-left corner.
           */
          let labelX = citation.bbox?.x ?? 0
          let labelY = citation.bbox?.y ?? 0
          ...
          if (citation.overlay?.points.length) {
            ...
            if (citation.overlay.type === "line") {
              ...
              labelX = Math.max(2, middle.x - 0.12)
              labelY = Math.max(4, middle.y - 0.09)

              // Bridge-specific positioning gives the label
              // enough room above the actual structure.
              if (citation.label.toLowerCase().includes("bridge")) {
                labelX = 0.18
                labelY = 0.70
              }
            } else {
          ...
          return (
              <div key={citation.id}>
                {/* Leader line from label to feature */}
                {citation.overlay?.type === "line" && (
                    <svg
                      ...
                      <line
                          ...
                          x2={
                              citation.overlay.points[
                                  Math.floor(citation.overlay.points.length / 2)
                                  ].x * 100
                          }
                          y2={
                              citation.overlay.points[
                ...
                <button
                    type="button"
                    onClick={() =>
                        onPick(active ? null : citation.id)
                    }
...
/** Draggable before/after clip comparison for bitemporal scenes. */
...
export function ImageViewer() {
  ...
  const setActiveCitation = useSatStore(
      (s) => s.setActiveCitation,
  )

  const [pairView, setPairView] = useState<
      "optical" | "sar"
  >("optical")

  const accent = intent
      ? SPECIALISTS[intent].accent
      : "#22d3ee"

  const showOverlays =
      status === "done" &&
      citations.some((c) => c.bbox || c.overlay)

  const visibleCitations = citations.filter((c) => {
    if (!c.bbox && !c.overlay) return false

    if (scene.mode === "pair") {
      return (c.target ?? "optical") === pairView
    }
      ...
      <div className="flex h-full flex-col">
        ...
        <p className="mt-2 text-center font-mono text-[10px] text-muted-2">
          {scene.mode === "bitemporal"
              ...
              : scene.mode === "pair"
                  ? "Toggle optical / SAR · overlays follow the active modality"
                  : "Click an evidence label to focus the grounded feature"}
        </p>
      </div>
  )
}
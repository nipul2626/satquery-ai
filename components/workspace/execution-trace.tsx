...
function StatusIcon({
  ...
  if (
    ...
    return (
        <Minus className="h-3.5 w-3.5 text-muted-2" />
    )
  }

  return (
      <Circle className="h-2.5 w-2.5 text-muted-2" />
  )
}
      ...
      <div>
        ...
        {!started ? (
            <p className="glass rounded-2xl p-4 text-xs leading-relaxed text-muted-2">
              The observable pipeline parses the
              request, retrieves supporting evidence,
              inspects the actual imagery, searches
              visual candidates, and returns grounded
              geometry. No hidden monologue is shown.
            </p>
        ) : (
